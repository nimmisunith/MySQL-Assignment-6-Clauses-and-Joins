...
SELECT SUM(e.salary) AS Total_Finance_Salary FROM Employees e
JOIN Departments d
...
COUNT(employee_id) AS Employee_Count FROM Employees
GROUP BY department_id HAVING COUNT(employee_id) < 3;

SELECT location_id,
AVG(age) AS Average_Age FROM Employees
WHERE gender = 'F' GROUP BY location_id
HAVING AVG(age) < 30;
